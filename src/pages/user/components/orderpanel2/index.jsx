...
import Enter from '../../../../imgs/nav_enter.png'
import host from '../../../../utils/host'
import allapi from '../../../../api';

export default class Orderpanel2 extends Component{
    constructor () {
        super(...arguments)
    }
    state={
    ...
    render(){
        const {orderStatusCode,claimsStatus,agentName,imgOne,createTm,productName,productAreas,marketPrice,shipCost,receiverName,receiverTel,receiverAddress,automaticReceipt,automaticReceiptTm,isReceipt}=this.props.orderdatas;
        let progressWord=''
        if(claimsStatus == '1'){
            progressWord='待处理'
        }else if(claimsStatus == '2'){
            progressWord='处理中'
        ...
        return(
            ...
            <AtToast isOpened={this.state.showToast} text={this.state.toastWord} duration={1500} onClose={this.closeToast}></AtToast>
                <View className='orderpanel-top' onClick={(claimsStatus!='' && orderStatusCode == null)?this.gotoRefundDetailPage:this.gotoOrderDetailPage}>
                    <View className='orderpanel-top-1'>
                        <View className='orderpanel-top-1-people'>提交人：{agentName}</View>
                        {orderStatusCode == null?<View className={'label'+(progressWord == '待处理'?' wait':'')+(progressWord == '处理中'?' doing':'')+(progressWord == '已处理'?' handled':'')+(progressWord == '已完结'?' done':'')}>{progressWord}</View>:<View className='orderpanel-top-1-time'>下单时间：{createTm}</View>}
                    </View>
                    <View className={'orderpanel-top-2'+(orderStatusCode==11||orderStatusCode == null?" no-action":'')} >
                        ...
                        <View className='orderpanel-top-2-infor'>
                            ...
                            <View className='orderpanel-top-2-infor-orgin'>
                                <View className='orderpanel-top-2-infor-orgin-word'>收货人：{receiverName}</View>
                            </View>
                            
                        </View>
                        
                        <View className='orderpanel-top-2-price'><View className='enter-word'>{(claimsStatus!='' && this.props.isDaka == true && orderStatusCode == null)? '查看详情':''}</View><Image src={Enter} className='enter-img'></Image></View>
                    </View>
                ...
                <View className='orderpanel-bottom' >
                {/* style={{height:(orderStatusCode == 11 || this.props.isDaka == false)?'80px':''}} */}
                    ...
                    {(orderStatusCode==2 && this.props.isDaka == true)?<View className='orderpanel-bottom-2' >
                        <View className='orderpanel-bottom-2-btn2'>
                            <View className='orderpanel-bottom-2-btn-word2' onClick={this.gotoModifyPage}>修改地址</View>
                        </View>
                        <View className='orderpanel-bottom-2-btn2'>
                    ...
                    <View className='orderpanel-bottom-2' style={{display: (orderStatusCode == 7 && this.props.isDaka == true) ? "" : "none"}}>
                        <View className='orderpanel-bottom-2-btn2'>
                            <View className='orderpanel-bottom-2-btn-word2' onClick={this.gotoWuliuPage}>查看物流</View>
                        </View>
                        ...
                        <View className='orderpanel-bottom-2-btn'>
                            {automaticReceipt?<View className='confirm-box'>
                                {isReceipt?<View>已签收</View>:<View>未签收</View>}
                                <View className='time'>{automaticReceiptTm}后</View>
                                <View>自动确认收货</View>
                            </View>:''}
                    ...
                    {/* <View className='orderpanel-bottom-2' style={{display: (claimsStatus!='' && this.props.isDaka == true && orderStatusCode == null)? "" : "none"}}>
                        ...
                        <View className="orderpanel-bottom-2-red"></View>
                    </View> */}
                </View>
